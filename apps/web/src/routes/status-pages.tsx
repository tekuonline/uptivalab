...
export const StatusPagesRoute = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: { name: string; slug: string; heroMessage?: string; monitorIds?: string[]; showIncidents?: boolean; showMaintenance?: boolean }) =>
      api.createStatusPage(token, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["status-pages"] });
      setForm({ name: "", slug: "", heroMessage: "", monitorIds: [], showIncidents: true, showMaintenance: true });
      setShowForm(false);
      setEditingId(null);
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: Partial<{ name: string; slug: string; heroMessage?: string; monitorIds?: string[]; showIncidents?: boolean; showMaintenance?: boolean }> }) =>
    ...
    <div className="space-y-6">
      <Card>
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h3 className="text-2xl font-semibold text-white">{t("statusPages")}</h3>
            <p className="text-sm text-slate-400">{t("publishTransparency")}</p>
          </div>
      ...
      {showForm && (
        <Card className="space-y-4">
          ...
          <div className="space-y-4">
            <div>
              <label className="text-sm text-slate-400 block mb-2">{t("pageName")}</label>
              <input
            ...
            <div>
              <label className="text-sm text-slate-400 block mb-2">{t("slug")}</label>
              <input
                className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white"
            ...
            <div>
              <label className="text-sm text-slate-400 block mb-2">{t("heroMessage")} ({t("optional")})</label>
              <textarea
                className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white"
            ...
            <div>
              <label className="text-sm text-slate-400 block mb-2">{t("monitorsToInclude")}</label>
              <div className="space-y-2">
                {monitors?.map((monitor) => (
                  <label key={monitor.id} className="flex items-center gap-3 rounded-lg border border-white/10 bg-white/5 px-4 py-2 cursor-pointer hover:bg-white/10">
                    <input
                      type="checkbox"
                ...
                {(!monitors || monitors.length === 0) && (
                  <p className="text-sm text-slate-500">{t("noMonitorsAvailable")}</p>
                )}
              </div>
            </div>
            <div className="space-y-3">
              <label className="text-sm text-slate-400 block">{t("displayOptions")}</label>
              <label className="flex items-center gap-3 rounded-lg border border-white/10 bg-white/5 px-4 py-3 cursor-pointer hover:bg-white/10">
                <input
                  type="checkbox"
                ...
                />
                <div>
                  <span className="text-sm font-medium text-white">{t("showIncidents")}</span>
                  <p className="text-xs text-slate-500">{t("showIncidentsDescription")}</p>
                </div>
              </label>
              <label className="flex items-center gap-3 rounded-lg border border-white/10 bg-white/5 px-4 py-3 cursor-pointer hover:bg-white/10">
                <input
                ...
                />
                <div>
                  <span className="text-sm font-medium text-white">{t("showUpcomingMaintenance")}</span>
                  <p className="text-xs text-slate-500">{t("showMaintenanceDescription")}</p>
                </div>
              </label>
            </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
          ...
          <Card key={page.id} className="space-y-2">
            <div className="flex items-start justify-between">
              <div>
                ...
                <h4 className="text-xl font-semibold text-white">{page.name}</h4>
              </div>
            ...
            {page.heroMessage && <p className="text-sm text-slate-400">{page.heroMessage}</p>}
            <div className="flex gap-2">
        ...
        {(data?.length ?? 0) === 0 && !showForm && (
          <p className="text-sm text-slate-400">{t("noStatusPages")}</p>
        )}
      </div>
    </div>
  );
