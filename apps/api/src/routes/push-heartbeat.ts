...
const heartbeatManagementPlugin = async (fastify: FastifyInstance) => {
  ...
  fastify.get("/heartbeats", async () => {
    return prisma.heartbeatToken.findMany({
  ...
  fastify.delete("/heartbeats/:id", async (request, reply) => {
  ...
  fastify.get("/heartbeats/:id/history", async (request) => {
    const params = z.object({ id: z.string() }).parse(request.params);
    const query = z.object({
      days: z.coerce.number().min(1).max(90).default(7),
    }).parse(request.query);

    const heartbeat = await prisma.heartbeatToken.findUniqueOrThrow({
      where: { id: params.id },
      include: { monitor: true },
    });
